...
const APP_RUNTIME_VERSION = process.env.DL_VERSION || "8790-125";
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (requestUrl.pathname === "/api/login" && req.method === "POST") {
      ...
      const loginGpsReject = loginGps ? gpsRejectReason(loginGps) : "";
      if (loginGpsReject) {
    ...
    if (requestUrl.pathname === "/api/preventa/whatsapp-contact" && req.method === "POST") {
      ...
      writeCompactStateResponse(res, currentState, { contact }, auditEntry(req, sessionUser, input, {
